    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <volist name="list" id="vo">
                <tr class="text-c" id="tr{$vo.id}"> 
                    <td><input type="checkbox" value="{$vo.id}" name="" class="check"></td>
                    <td>{$vo.id}</td>
                    <td style=" text-align: left">{$vo.title}</td>
                    <td>{$vo.nickname}</td>
                    <td>{$vo.text}</td>
                    <td>{$vo.time}</td>
<!--                            <if condition="$vo[statu] eq 0">
                        <td><span class="btn btn-success">已发布</span></td>
                    <else/>
                    ...
                    <td><!--显示审核状态-->
                        ...
                        <button onclick="statu1(this,{$vo[id]})" class='btn btn-danger radius'>提交</button><!--{$vo[id]}  equal {$vo.id}-->
                    </td>
                    
<!--                            <td class="td-manage">
                        <a title="编辑" href="/home/admin/menu/id/{$vo.id}" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
                        <a title="删除" href="/home/admin/menudelete/id/{$vo.id}" class="ml-5  jq-del"  data-id="{$vo.id}" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>-->
                </tr>
            </volist>
...
/*选择全部栏目*/
  ...
  $("#del_all").click(function datadel()
 {
    ...
    {
        //发送Ajax请求
        $.post("{:U('ajax_delAllMenu')}",{str:str},function(data)
        {
            if(data == 0)
            {
                alert("删除失败");
