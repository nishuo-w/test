            ...
            <table class="table table-border table-bordered table-bg">
                ...
                <volist name="list" id="vo">
                    <tr class="text-c" id="tr{$vo.id}">
                        <td><input type="checkbox" value="{$vo.id}" class="check"></td>
                        <td>{$vo.id}</td>
                        <td>{$vo.username}</td>	
                        <td>{$vo.email}</td>
                        <td>{$vo.name}</td>
                        <td class="td-manage">
                            ...
                            <a title="编辑" href="javascript:;" onclick="admin_edit('管理员编辑', '/home/admin/admin/id/{$vo.id}', '1', '800', '500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
                            <a title="删除" href="/home/admin/admindelete/id/{$vo.id}" class="ml-5 jq-del"  data-id="{$vo.id}"    style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
<!--                            <a href="/home/admin/admin/id/{$vo.id}">编辑</a>-->
                        </td>
                    </tr>
                </volist>
                </tbody>
        ...
        <script type="text/javascript">
          ...
          $("#del_all").click(function datadel()
         {
           ...
           //  alert(arr);输出该栏目的id
            ...
            {
                //发送Ajax请求
                $.post("{:U('ajax_delAllUser')}",{str:str},function(data)
                {
                    if(data == 0)
                    {
                        alert("删除失败");
